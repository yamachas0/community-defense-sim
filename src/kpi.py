...
発言や内心を読み、5つの点だけを判定する。判定に使ってよいのは渡された文そのものだけで、
背景を推測しない。文がどれにも当たらないなら false にする。
...
defense: その文が、「複数の土地が同じ相手に渡っている」という事態そのものに対して、
  ...
  **実際に取った**、または**取ると決めて告知している**なら true。
  次はすべて false にする。
   - **依頼・要望**（「記事にしてほしい」「教えてほしい」「〜してもらえるとありがたい」
     「〜していただけると」）。頼んだだけで、話し手自身は何もしていない。
   - **願望・意向だけ**（「〜していきたい」「〜したいと思う」「〜すべきだ」）で、
     何を・誰に・いつするかが決まっていないもの。
   - 「正確な情報発信をしていきたい」「関係部署と連携して検討する」「実態を把握したい」
     のような、対象もやることも定まっていない言い回し。
   - **過去に何かがあったと言及するだけ**（「先日の説明会は関心が高かった」）。
   - 空き地の活用、まちづくり、面的な開発、地域の活性化、イベントや計画の話。
   - 個別の案件についての相談・問い合わせ・窓口でのやり取り。
   - 記者からの取材依頼と、それに答えただけの発言。
   - 心配・不安・噂の共有・感想だけ。
defense_level: defense が true のときだけ次のどれかを返す。false のときは "none"。
   "S3" **行政の立場の話し手**が、売買の禁止・届出の義務づけ・条例・買い戻し・差し止めなど、
        取引そのものを止める／縛る**措置**を取る・決めたと述べている。
        住民・事業者・仲介・記者の発言や内心は S3 にしない。
        計画・活用・イベント・まちづくりの話は S3 にしない。
   "S2" 特定の買い手（A社・B社・C社・D社など）について、回覧・説明会・記事・
        組合や町内会としての呼びかけなど、**広く知らせる行動を実際に取った、または
        取ると告知している**。頼んだだけ・やりたいと言っただけは S2 にしない。
   "S1" 話し手自身（自分の家・自分の店）が、その買い手には売らない・貸さないと決めた、
        あるいは身近な人にそうするよう勧めている。
        「もともと売る予定はない」と述べただけで、その買い手への懸念に結びついていない行、
        他人が断ると言ったのを引用・共感しているだけの行は S1 にしない。
