#!/usr/bin/env python
"""v8e — エントリポイント（「海外」→「市外」の1点だけを変えた版）。

  python run_v8e.py --config configs/config_field_v8e.yaml --provider mock
  python run_v8e.py --config configs/config_field_v8e.yaml --steps 1     # 実APIスモーク
  python run_v8e.py --config configs/config_field_v8e.yaml --workers 4   # 本走（36か月）
  python run_v8e.py --intro-mode overseas ...   # v8d と1文字も同じ文面に戻す

既存の `run.py`・`run_v8*.py`（v8/v8b/v8c/v8d）には触らない。
**CEO の GO が出るまで実APIでは走らせない**（施主 2026-08-30 01:37 の準備指示）。
"""

from __future__ import annotations

import argparse
import json
import logging
import os
import shutil
import sys

import yaml

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from run_v8c import ROOT, load_dotenv, resolve_run_dir   # noqa: E402
from src.sim_v8e import SimulationV8E                    # noqa: E402


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--config", default="configs/config_field_v8e.yaml")
    ap.add_argument("--provider", default=None, help="google | mock")
    ap.add_argument("--steps", type=int, default=None)
    ap.add_argument("--run-name", default=None)
    ap.add_argument("--intro-mode", default=None,
                    choices=["overseas", "city_outside"])
    ap.add_argument("--no-cache", action="store_true")
    ap.add_argument("--max-cost", type=float, default=None)
    ap.add_argument("--out-dir", default=None)
    ap.add_argument("--workers", type=int, default=None)
    ap.add_argument("--timeout", type=float, default=None)
    ap.add_argument("--quiet", action="store_true")
    args = ap.parse_args()

    logging.basicConfig(
        level=logging.WARNING if args.quiet else logging.INFO,
        format="%(asctime)s %(levelname)s %(message)s")
    logging.getLogger("google").setLevel(logging.WARNING)
    logging.getLogger("httpx").setLevel(logging.WARNING)
    load_dotenv()

    cfg_path = (args.config if os.path.isabs(args.config)
                else os.path.join(ROOT, args.config))
    with open(cfg_path, encoding="utf-8") as f:
        cfg = yaml.safe_load(f)
    if args.provider:
        cfg["llm"]["provider"] = args.provider
    if args.steps:
        cfg["steps"] = int(args.steps)
    if args.intro_mode:
        cfg["acquirer_intro_mode"] = args.intro_mode
    if args.no_cache:
        cfg["llm"]["enable_cache"] = False
    if args.max_cost is not None:
        cfg["max_cost_usd"] = float(args.max_cost)
    if args.workers:
        cfg["llm"]["parallel_workers"] = int(args.workers)
    if args.timeout is not None:
        cfg["llm"]["request_timeout_sec"] = float(args.timeout)
    if args.run_name:
        cfg["run_name"] = args.run_name

    run_dir = resolve_run_dir(str(cfg["run_name"]), args.out_dir)
    shutil.copy(cfg_path, os.path.join(run_dir, "config.yaml"))
    print(f"run_dir: {run_dir}", flush=True)
    sim = SimulationV8E(cfg, run_dir)
    summary = sim.run()
    print(json.dumps({k: v for k, v in summary.items() if k != "usage"},
                     ensure_ascii=False, indent=2))
    print(f"run_dir: {run_dir}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
