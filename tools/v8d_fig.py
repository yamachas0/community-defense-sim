...
def main() -> int:
    ...
    ap.add_argument("--v8b", default=None, help="前の版の run_dir（薄い破線で重ねる）")
    ap.add_argument("--out", default="docs/submission/fig_v8d_curve.png")
    args = ap.parse_args()
    ...
    s = chat["summary"]
    n_parcels = int(s.get("acquired_parcels", 0))
    n_agents = int(s.get("sold_agents", 0))
    n_months = int(s.get("months_run", 0))
    if n_parcels == 0:
        note = f"今回は{n_months}か月ずっと0件"
    else:
        note = (f"今回は{n_months}か月で{n_parcels}区画・{n_agents}人")
    ax.set_title(f"町の不動産が買い手の名義に移っていく速さ（{note}）",
                 fontsize=13, pad=12)
