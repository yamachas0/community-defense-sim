...
"""v9 の曲線図（決定論・API 不使用）。
  ...
  python tools/v9_fig.py --run simulations/<v9_run> \
      --v8d simulations/<v8d_run> --out docs/submission/fig_v9_curve.png

上の段＝X社の所有権が及んだ区画の数（土地だけ／建物だけ／両方の内訳を積み上げ）と、
v8d の同じ町での結果を薄い破線で重ねたもの。
...
def main() -> int:
    ...
    ap.add_argument("--v8d", default=None)
    ap.add_argument("--out", default="docs/submission/fig_v9_curve.png")
    args = ap.parse_args()
    ...
    if args.v8d:
        vs, vm = _read(args.v8d)
        ax.plot([0] + [x["step"] for x in vm],
                [0] + [x.get("parcels_cum", 0) for x in vm],
                color="#888888", lw=1.8, ls="--",
                label="前の版（v8d・同じ町・土地と建物を分けない世界）")
