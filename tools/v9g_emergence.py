#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""v9g の観測をぜんぶ機械で数える（決定論・API を一切使わない・採点 LLM なし）。
...
def main() -> int:
    ...
    ap.add_argument("--out", default="docs/submission/emergence_v9g.json")
    args = ap.parse_args()
    r = args.run_dir
    ...
    # --- v9g の観測（網羅率・町の反応） --------------------------------------
    cov = load(r, "coverage.json", [])
    out["coverage"] = {
        "mean": s.get("coverage_mean"),
        "full_months": s.get("coverage_full_months"),
        "monthly": cov,
    }

    # 走行前に凍結した「町の反応」の語（設計 docs/world_design_v9g.md §2-2）
    TOWN_WORDS = ["買い占め", "警戒", "不安", "みんなで", "皆さんのところにも",
                  "意図", "危機"]
    SUSPICION_WORDS = ["買い占め", "警戒", "意図", "危機"]

    def hits(texts):
        return word_counts([t for t in texts], TOWN_WORDS)
    ...
    for u in utter:
        for w in TOWN_WORDS:
            if w in str(u.get("text", "") or ""):
                rows_verbatim.append({"where": "発言", "month": u["step"],
                                      "who": u["from"], "word": w,
                                      "text": u["text"]})
                break
    for u in utter:
        t = str(u.get("thought", "") or "")
        for w in TOWN_WORDS:
            if w in t:
                rows_verbatim.append({"where": "内心", "month": u["step"],
                                      "who": u["from"], "word": w, "text": t})
                break
    for d0 in decline:
        for w in TOWN_WORDS:
            if w in str(d0["text"] or ""):
                rows_verbatim.append({"where": "断りの理由", "month": d0["month"],
                                      "who": d0["to"], "word": w,
                                      "text": d0["text"]})
                break
    for x in listing_reasons:
        for w in TOWN_WORDS:
            if w in str(x["text"] or ""):
                rows_verbatim.append({"where": "出品の理由", "month": x["month"],
                                      "who": x["who"], "word": w,
                                      "text": x["text"]})
                break

    out["town_reaction"] = {
        "vocab_frozen_before_the_run": TOWN_WORDS,
        "utterances": hits(utter_texts),
        "thoughts": hits(thought_texts),
        "decline_reason": hits(decline_texts),
        "listing_reason": hits(listing_texts),
        "sell_reason": hits(sell_texts),
        "rows_verbatim": rows_verbatim,
        "suspicion_declines": {
            "vocab": SUSPICION_WORDS,
            "count": len([d0 for d0 in decline
                          if any(w in str(d0["text"] or "")
                                 for w in SUSPICION_WORDS)]),
            "rows": [{"month": d0["month"], "who": d0["to"], "text": d0["text"]}
                     for d0 in decline
                     if any(w in str(d0["text"] or "") for w in SUSPICION_WORDS)],
        },
    }
    ...
    print(json.dumps({"out": args.out,
                      ...
                      "town_words": out["town_reaction"]["utterances"]["total_hits"]
                                     + out["town_reaction"]["decline_reason"]["total_hits"],
