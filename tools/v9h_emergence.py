#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""v9h の観測をぜんぶ機械で数える（決定論・API を一切使わない・採点 LLM なし）。
...
def main() -> int:
    ...
    ap.add_argument("--out", default="docs/submission/emergence_v9h.json")
    args = ap.parse_args()
    r = args.run_dir
    ...
    # --- v9h の観測（町の反応・明言の届き方） --------------------------------
    # 語はすべて走行前に凍結（設計 docs/world_design_v9h.md §2）。分類はしない。
    REACT_WORDS = ["買い占め", "警戒", "不安", "みんなで", "皆さんのところにも",
                   "意図", "危機"]
    SCALE_WORDS = ["過半", "街全体", "町ごと", "支配", "乗っ取"]
    SUSPICION = ["買い占め", "警戒", "意図", "危機", "支配", "乗っ取"]
    GROUP_WORDS = ["自治会", "申し合わせ", "みんなで断", "住民同士", "協力して"]
    ALL_WORDS = REACT_WORDS + SCALE_WORDS

    utter_texts = [u.get("text", "") for u in utter]
    thought_texts = [u.get("thought", "") for u in utter]
    decline_texts = [d["text"] for d in decline]
    listing_texts = [x["text"] for x in listing_reasons]
    sell_texts = [x["text"] for x in sell_reasons]

    def verbatim(where, rows, key, who_key, month_key):
        out_rows = []
        for x in rows:
            t = str(x.get(key, "") or "")
            hit = [w for w in ALL_WORDS if w in t]
            if hit:
                out_rows.append({"where": where, "month": x[month_key],
                                 "who": x[who_key], "words": hit, "text": t})
        return out_rows

    rows_verbatim = (
        verbatim("発言", utter, "text", "from", "step")
        + verbatim("内心", utter, "thought", "from", "step")
        + verbatim("断りの理由", decline, "text", "to", "month")
        + verbatim("出品の理由", listing_reasons, "text", "who", "month")
        + verbatim("売る理由", sell_reasons, "text", "who", "month"))

    out["town_reaction"] = {
        "vocab_frozen_before_the_run": {"reaction": REACT_WORDS,
                                        "scale": SCALE_WORDS,
                                        "suspicion": SUSPICION,
                                        "group": GROUP_WORDS},
        "reaction_words": {
            "utterances": word_counts(utter_texts, REACT_WORDS),
            "thoughts": word_counts(thought_texts, REACT_WORDS),
            "decline_reason": word_counts(decline_texts, REACT_WORDS),
            "listing_reason": word_counts(listing_texts, REACT_WORDS),
            "sell_reason": word_counts(sell_texts, REACT_WORDS),
        },
        "scale_words": {
            "utterances": word_counts(utter_texts, SCALE_WORDS),
            "thoughts": word_counts(thought_texts, SCALE_WORDS),
            "decline_reason": word_counts(decline_texts, SCALE_WORDS),
            "listing_reason": word_counts(listing_texts, SCALE_WORDS),
            "sell_reason": word_counts(sell_texts, SCALE_WORDS),
        },
        "group_words": {
            "utterances": word_counts(utter_texts, GROUP_WORDS),
            "thoughts": word_counts(thought_texts, GROUP_WORDS),
            "decline_reason": word_counts(decline_texts, GROUP_WORDS),
        },
        "suspicion_declines": {
            "count": len([d0 for d0 in decline
                          if any(w in str(d0["text"] or "") for w in SUSPICION)]),
            "rows": [{"month": d0["month"], "who": d0["to"], "text": d0["text"]}
                     for d0 in decline
                     if any(w in str(d0["text"] or "") for w in SUSPICION)],
        },
        "rows_verbatim": rows_verbatim,
        "note": "発言・内心・断り・出品理由を分けて数えている。内心は誰にも伝わらないので、"
                "合計を「町で共有された危機感」と読んではいけない。",
    }

    out["declaration"] = {
        "text": s.get("declaration"),
        "offers_delivered_total": s.get("offers_delivered_total"),
        "offers_with_declaration": s.get("offers_with_declaration"),
    }
    ...
    print(json.dumps({"out": args.out,
                      ...
                      "suspicion_declines":
                          out["town_reaction"]["suspicion_declines"]["count"],
                      "reaction_rows": len(out["town_reaction"]["rows_verbatim"]),
