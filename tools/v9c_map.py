#!/usr/bin/env python
"""v9c の町の地図（平面図と断面図）。決定論・API を一切使わない。

  # 開始時（第1月）＝名簿だけから描ける
  python tools/v9c_map.py --personas configs/personas_v9.yaml --month 1 \
      --out-dir docs/submission

  # 走行のあと（第12・24・36月）
  python tools/v9c_map.py --run simulations/<v9c_run_dir> --months 12,24,36 \
      --out-dir docs/submission

出すもの（施主指示 2026-08-30 11:06）:
  ① 平面 `fig_v9c_map_plan_mNN.png`   … 44区画の配置図（隣近所と同じ格子）。
     土地の色＝土地の持ち主の種類、建物の色＝建物の持ち主の種類、借りている人は印。
  ② 断面 `fig_v9c_map_section_mNN.png` … 権利の重なりを正面から見た帯の図。
...
def main() -> int:
    ...
    for m in [int(x) for x in args.months.split(",") if x.strip()]:
        ...
        if args.which in ("section", "both"):
            out = os.path.join(args.out_dir, f"fig_v9c_map_section_m{m:02d}.png")
            print("wrote", draw_section(rows, kinds, m, out, codes,
                                        n_town, n_absent))
        if args.which in ("plan", "both"):
            out = os.path.join(args.out_dir, f"fig_v9c_map_plan_m{m:02d}.png")
