...
"""v9d — エントリポイント。
  ...
  python run_v9d.py --config configs/config_field_v9d.yaml --provider mock
  python run_v9d.py --config configs/config_field_v9d.yaml --steps 1     # 実APIスモーク
  python run_v9d.py --config configs/config_field_v9d.yaml --workers 4   # 本走（36か月）

既存の `run.py`・`run_v8*.py`・`run_v9.py`・`run_v9b.py`・`run_v9c.py` には触らない。
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/config_field_v9d.yaml")
