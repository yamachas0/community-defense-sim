...
"""v9b — エントリポイント。
  ...
  python run_v9b.py --config configs/config_field_v9b.yaml --provider mock
  python run_v9b.py --config configs/config_field_v9b.yaml --steps 1     # 実APIスモーク
  python run_v9b.py --config configs/config_field_v9b.yaml --workers 4   # 本走（36か月）

既存の `run.py`・`run_v8*.py`・`run_v9.py` には触らない。
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/config_field_v9b.yaml")
