...
check("S3 > S2 > S1 の順で当たる",
      ...
      and rule_defense_level("回覧に載せて皆に知らせた。うちは売らない") == "S2"
...
# --- 締め直し後の S2/S3（施主指示 2026-08-29・第1便の偽陽性3行を落とす）------
# 定義は変えていない。実装（語彙とLLM指示）を定義に届かせただけ。
# ここで固定するのは**ルール側**（LLM 指示の締めは実APIでしか確かめられない）。
FP_B = "前回の町内会での説明会、皆さん関心が高かったようですね。特にD社の動きとスペースポート構想の関連について、色々と質問がありました。住民の方々がより安心して今後の街の動きを理解できるよう、正確な情報発信をしていきたいのですが。"
FP_C = "岬道の登記問題、本当に街の未来に関わることだと思うんです。記者さんにも、この問題の重要性を記事にしてもらえるとありがたいですね。"
FP_C_S3 = "岬道の登記問題、空き店舗活用、湾岸大通りマルシェ計画について、具体的な情報共有と連携の必要性を感じている。記者にマルシェ計画の空き店舗活用への繋がりを伝え、前向きな記事にしてもらうことで、関係者の関心を高めたい。"

check("偽陽性B（行政の『正確な情報発信をしていきたい』）はルール側で赤にならない",
      rule_defense_level(FP_B, "municipality") is None, str(rule_defense_level(FP_B)))
check("偽陽性C（記者への『記事にしてもらえるとありがたい』＝依頼）は赤にならない",
      rule_defense_level(FP_C, "household") is None, str(rule_defense_level(FP_C)))
check("偽陽性C-S3（住民の内心・マルシェ計画）は S3 にならない",
      rule_defense_level(FP_C_S3, "household") != "S3",
      str(rule_defense_level(FP_C_S3, "household")))
check("S3 は行政の立場にしか付かない",
      rule_defense_level("売買を禁止する条例を制定した", "municipality") == "S3"
      and rule_defense_level("売買を禁止する条例を制定した", "household") != "S3")
check("LLM が S3 と言っても行政でなければ S3 にしない",
      (defense_level_of({"classified": True, "rule_red": True, "rule_yellow": False,
                         "rule_green": False, "rule_blue": True, "llm_defense": True,
                         "llm_defense_level": "S3", "role": "household",
                         "text": "A社には売らない"}) or {}).get("level") == "S1")

# runA の真陽性（第6月の停止トリガー）は S1 のまま
TP_A = ["D社の買収の件、私も気になっています。もし何か情報があれば、ぜひ記事にしてください。私の土地は手放すつもりはないので、もしD社から接触があったら、その旨をお伝えください。",
        "魚市場前の持ち主さん、D社の買収の噂、気になりますよね。もしD社から接触があったら、きっぱり断ろうと思っています。"]
check("runA の停止を支えた行（「きっぱり断ろう」）は S1 のまま",
      rule_defense_level(TP_A[1], "household") == "S1",
      str(rule_defense_level(TP_A[1], "household")))
# **既知の偽陰性（正直に固定する）**：定義上は S1 に当たる言い回しでも、凍結した語彙に
# 無い形（「手放すつもりはない」「売るつもりはない」）はルール側で拾えない。
# S1 の語彙は施主指示により締め直しの対象外なので、**結果を見てから足すことはしない**。
# runA の停止は「きっぱり断ろう」の2行で成立しており、この偽陰性では覆らない。
check("既知の偽陰性：「手放すつもりはない」はルール側では拾えない",
      rule_defense_level(TP_A[0], "household") is None,
      str(rule_defense_level(TP_A[0], "household")))
...
check("LLM が none ならルール側の最上位を使う",
      defense_level_of(red_row("回覧に載せて皆に知らせた", "none"))
