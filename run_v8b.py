...
"""v8b「売りに出す町」— エントリポイント。
  ...
  python run_v8b.py --config configs/config_field_v8b.yaml --provider mock
  python run_v8b.py --config configs/config_field_v8b.yaml --steps 1     # 実APIスモーク
  python run_v8b.py --config configs/config_field_v8b.yaml               # 本走（36か月）

既存の `run.py`・`run_v8.py` には触らない（v1〜v8 の実行経路はそのまま）。
...
from src.sim_v8b import SimulationV8B   # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/config_field_v8b.yaml")
