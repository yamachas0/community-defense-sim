...
"""v9f — エントリポイント。
  ...
  python run_v9f.py --config configs/config_field_v9f.yaml --provider mock
  python run_v9f.py --config configs/config_field_v9f.yaml --steps 1     # 実APIスモーク
  python run_v9f.py --config configs/config_field_v9f.yaml --workers 4   # 本走（36か月）

既存の `run.py`・`run_v8*.py`・`run_v9.py`・`run_v9b.py`・`run_v9c.py`・`run_v9d.py`・`run_v9e.py` には触らない。
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/config_field_v9f.yaml")
