#!/usr/bin/env python
"""v8c「売る／売らないと、その理由」— エントリポイント。

  python run_v8c.py --config configs/config_field_v8c.yaml --provider mock
  python run_v8c.py --config configs/config_field_v8c.yaml --steps 1     # 実APIスモーク
  python run_v8c.py --config configs/config_field_v8c.yaml               # 本走（36か月）

既存の `run.py`・`run_v8.py`・`run_v8b.py` には触らない（v1〜v8b の実行経路はそのまま）。
...
from src.sim_v8c import SimulationV8C   # noqa: E402
...
def resolve_run_dir(run_name: str, base: str = None) -> str:
    # v8 の run.py は連番を 2桁 (\d{2}) でしか拾えず 100 番以降が頭打ちになるので、
    # v8b/v8c では 2桁以上を拾う（v8 のファイルは触らない）。
    SIM = base or SIM_DIR
    ...
    path = os.path.join(SIM, f"{stamp}_{nn + 1:02d}_{run_name}")
...
def main() -> int:
    ...
    ap.add_argument("--config", default="configs/config_field_v8c.yaml")
    ...
    ap.add_argument("--out-dir", default=None,
                    help="出力先の親フォルダ（既定 simulations/。mock はここを外に向ける）")
